...
func CallDataDome(captchaURL string, Site string, region string) (error, string) {
	...
	req.Header.Set("Origin", fmt.Sprintf("https://www.%s.%s", Site, region))
	req.Header.Set("Sec-Fetch-Site", "cross-site")
	req.Header.Set("Sec-Fetch-Mode", "cors")
	req.Header.Set("Sec-Fetch-Dest", "empty")
	req.Header.Set("Referer", fmt.Sprintf("https://www.%s.%s/", Site, region))
	...
	if resp.StatusCode == 200 {
		...
		cookie, ok := dd["cookie"]
		if ok {
			DataDome := cookie.(string)[9:115]
			return nil, DataDome
		}
	}
	return errors.New("No Datadome"), ""
}
